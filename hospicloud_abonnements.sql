...
CREATE TABLE `abonnements` (
  ...
  `plan_nom` varchar(50) DEFAULT NULL,
  `montant_mensuel` decimal(10,2) DEFAULT NULL,
  `statut` enum('actif','annule','suspendu') DEFAULT 'actif',
  `date_debut` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `date_fin` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id_abonnement`),
  KEY `id_hopital` (`id_hopital`),
  CONSTRAINT `abonnements_ibfk_1` FOREIGN KEY (`id_hopital`) REFERENCES `hopitaux` (`id_hopital`)
