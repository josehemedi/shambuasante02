...
CREATE TABLE `dispensations` (
  ...
  `statut_paiement` enum('PAYE','EN_ATTENTE','PRIS_EN_CHARGE') DEFAULT 'EN_ATTENTE',
  PRIMARY KEY (`id_dispensation`),
  KEY `id_patient` (`id_patient`),
  KEY `id_consultation` (`id_consultation`),
  KEY `id_pharmacien` (`id_pharmacien`),
  CONSTRAINT `dispensations_ibfk_1` FOREIGN KEY (`id_patient`) REFERENCES `patients` (`id_patient`) ON DELETE CASCADE,
  CONSTRAINT `dispensations_ibfk_2` FOREIGN KEY (`id_consultation`) REFERENCES `consultations` (`id_consultation`) ON DELETE SET NULL,
  CONSTRAINT `dispensations_ibfk_3` FOREIGN KEY (`id_pharmacien`) REFERENCES `utilisateurs` (`id_utilisateur`) ON DELETE CASCADE
